...
the single-page build, plus `public/manifest.webmanifest` beside it. Both are
committed; the icons they point at are generated by `pnpm icons`, which the web
build runs. Two things there are easy to lose and neither fails loudly:
...
- **`output: "single"` means `app/+html.tsx` is never read.** Expo takes the
  ...
  it doesn't, so head tags belong in that file. A `+html.tsx` would be silently
  ignored.
...
public/         The web shell — index.html, manifest, generated icons
```
