...
| Home topic modes + per-mode starter questions    | ✅     | `home.tsx`, `lib/starters.ts`         | [Spec](./specs/starters.md). 8 modes, each a pool of 8–12 written starters, EN + HI; 3 drawn per visit, stable while shown, redrawn on choosing a chip. Adhyayan last, in its own vocabulary. Shown only while the composer is empty; tapping one sends it |
...
| Model picker: **Dhee Reflective (default) / Quick** | ✅     | `Composer`, `agents/models.ts` | Two tiers wired end to end; Reflective stays the default until Quick passes its eval gate. `Deep` deliberately unclaimed for the paid tier (Epic 14). `fastAnswers/higherIntel` toggles still stubs |
