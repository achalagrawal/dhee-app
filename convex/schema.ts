...
export default defineSchema({
  ...
  // One index, not two: a `by_user` here would be a prefix of
  // `by_user_concept`, which Convex's best-practices guide says to drop —
  // a range over just `userId` walks the same rows (ordered by conceptSlug
  // rather than creation time, which no reader of this table depends on),
  // and every extra index is paid for on every write.
